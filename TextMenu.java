...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem justify; // controls text justification
	...
	public TextMenu(DrawPanel owner) {
		...
		// line height menu item
				final JMenuItem lineHeight = new JMenuItem("set up line space");
				lineHeight.addActionListener( new ActionListener() {
					public void actionPerformed(ActionEvent evt) {
						double currentHeight = panel.getTextItem().getLineHeightMultiplier();
						String s = JOptionPane.showInputDialog(panel, "Multiply the default line spacing ?",currentHeight);
						if (s != null && s.trim().length() > 0) {
							try {
								...
								JOptionPane.showMessageDialog(panel, s + " is not a real line height.\n"
	...
	/**
	 * Creates the alignment menu setting up a radio button group
